          ...
          <tr>
            <td>
              <div style="display:flex;align-items:center;gap:10px">
                ...
                <div>
                  <div style="font-weight:600">${escapeHtml(c.nombres + ' ' + c.apellidos)}</div>
            ...
            <td>
              <div class="table-actions">
                <button class="btn btn-secondary btn-sm" onclick="viewHistorial('${c.id}', '${escapeHtml(c.nombres + ' ' + c.apellidos)}')" title="Ver historial">📋</button>
                ${canEdit ? `
                  <button class="btn btn-primary btn-sm" onclick="editColaborador('${c.id}')" title="Editar">✎</button>
                  <button class="btn btn-danger btn-sm" onclick="deleteColaborador('${c.id}', '${escapeHtml(c.nombres + ' ' + c.apellidos)}')" title="Eliminar">🗑</button>
                ` : ''}
              </div>
            </td>
          </tr>
...
function openModalColaborador(data = null) {
  ...
  document.getElementById('colabFechaIngreso').value = data?.fecha_ingreso || new Date().toISOString().split('T')[0];
...
async function saveColaborador() {
  ...
  try {
    if (id) {
      ...
      showNotification('success', 'Actualizado', `${nombres} ${apellidos} fue actualizado correctamente`);
    } else {
      ...
      showNotification('success', 'Registrado', `${nombres} ${apellidos} fue registrado correctamente`);
    }
