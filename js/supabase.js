...
function formatDate(date) {
  if (!date) return '-';
  // Parseo directo de YYYY-MM-DD — los campos DATE no tienen timezone
  const str   = String(date).substring(0, 10);
...
function formatDateTime(dt) {
  if (!dt) return '-';
  // Convierte TIMESTAMPTZ UTC de Supabase → hora Lima
...
function formatNombreCompleto(nombre) {
  if (!nombre) return '';
  if (nombre.includes(',')) return nombre; // ya está en formato "Apellidos, Nombres"
  const partes = nombre.trim().split(/\s+/);
  if (partes.length <= 1) return nombre;
  if (partes.length === 2) return `${partes[1]}, ${partes[0]}`;
  const apellidos = partes.slice(-2).join(' ');
  const nombresP  = partes.slice(0, -2).join(' ');
  return `${apellidos}, ${nombresP}`;
}

/* --- Helpers Lima (disponibles globalmente en todas las páginas) --- */

/** Fecha actual en Lima como YYYY-MM-DD — usar para filtros DATE en Supabase */
function getLimaISODate() {
  return new Date().toLocaleDateString('en-CA', { timeZone: 'America/Lima' });
}

/** ISO string UTC — formato correcto para guardar en Supabase TIMESTAMPTZ */
function getLimaISO() {
  return new Date().toISOString();
}

/** Objeto completo con fecha y hora Lima para registros y auditoría */
function getFechaHoraLima() {
  ...
  return { fecha, hora, fechaHora: `${fecha} ${hora}`, isoString: now.toISOString() };
}

/** Inicia un reloj Lima en tiempo real — actualiza cada segundo */
function iniciarRelojLima({ elFecha = null, elHora = null } = {}) {
  const tick = () => {
    const now = new Date();
    if (elFecha) {
      const t = now.toLocaleDateString('es-PE', {
        weekday: 'long', day: 'numeric', month: 'long', year: 'numeric',
        timeZone: 'America/Lima'
      });
      elFecha.textContent = t.charAt(0).toUpperCase() + t.slice(1);
    }
    if (elHora) {
...
/* --- Rangos de fecha usando hora Lima --- */

function getWeekRange(date = new Date()) {
  const limaStr  = new Date(date).toLocaleDateString('en-CA', { timeZone: 'America/Lima' });
  ...
  const start    = new Date(y, m - 1, d + toMon,     12, 0, 0);
  const end      = new Date(y, m - 1, d + toMon + 6, 12, 0, 0);
