...
async function init() {
  ...
  (collabs || []).forEach(c => {
    ...
    opt.value = c.id;
    opt.textContent = formatNombreCompleto(c.nombre_completo);
    sel.appendChild(opt);
  });

  // Defaults en hora Lima
  const limaDate = getLimaISODate(); // YYYY-MM-DD
  const limaYear = parseInt(limaDate.split('-')[0]);
  const limaMes  = limaDate.slice(0, 7); // YYYY-MM
  document.getElementById('filtroDia').value  = limaDate;
  document.getElementById('filtroMes').value  = limaMes;
  document.getElementById('filtroAnio').value = limaYear;

  // Semana actual en Lima
  const nowLima = new Date(limaDate + 'T12:00:00');
  const wNum    = getWeekNumber(nowLima);
  document.getElementById('filtroSemana').value = `${limaYear}-W${String(wNum).padStart(2, '0')}`;
          ...
          <tbody>
            ${data.map(v => `
              <tr>
                ...
                <td>
                  <div style="font-size:.8rem">${escapeHtml(formatNombreCompleto(v.conductor_resguardo || ''))}</div>
                  <div style="font-size:.75rem;color:var(--text-muted)">${escapeHtml(formatNombreCompleto(v.conductor_trailer || ''))}</div>
                </td>
...
function renderTabPagos() {
  ...
  data.forEach(r => {
    ...
    if (!byCollab[id]) {
      byCollab[id] = {
        nombre: formatNombreCompleto(r.colaboradores?.nombre_completo || ''),
...
async function exportarExcel() {
  ...
  if (reporteData.pagos.length > 0) {
    ...
    reporteData.pagos.forEach(r => {
      const id = r.colaborador_id;
      if (!byCollab[id]) byCollab[id] = { nombre: formatNombreCompleto(r.colaboradores?.nombre_completo || ''), dni: r.colaboradores?.dni, cargo: r.colaboradores?.cargo, viajes: 0, monto: 0 };
      byCollab[id].viajes++;
      byCollab[id].monto += parseFloat(r.pago) || 0;
    });
...
async function exportarPDF() {
  ...
  doc.text(`Generado: ${getFechaHoraLima().fechaHora} Lima | Usuario: ${Session.get()?.nombre_completo || '-'}`, 148.5, 24, { align: 'center' });
  ...
  if (reporteData.pagos.length > 0 && y < 180) {
    ...
    reporteData.pagos.forEach(r => {
      const id = r.colaborador_id;
      if (!byCollab[id]) byCollab[id] = { nombre: formatNombreCompleto(r.colaboradores?.nombre_completo || ''), dni: r.colaboradores?.dni || '-', viajes: 0, monto: 0 };
      byCollab[id].viajes++;
      byCollab[id].monto += parseFloat(r.pago) || 0;
    });
...
async function generarBackupMensual() {
  ...
  showNotification('success', 'Backup Mensual', `Backup del mes ${new Date().toLocaleString('es-PE', { month: 'long', year: 'numeric', timeZone: 'America/Lima' })} generado`);
...
async function verificarBackupAutomatico() {
  const limaStr = getLimaISODate(); // YYYY-MM-DD Lima
  const [ly, lm, ld] = limaStr.split('-').map(Number);
  const hoy     = { getDate: () => ld, getFullYear: () => ly, getMonth: () => lm - 1 };
