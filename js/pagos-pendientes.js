              ...
              <div style="display:flex;align-items:center;gap:10px">
                ...
                <div>
                  <div style="font-weight:700;color:var(--text-primary)">
                    ${escapeHtml(formatNombreCompleto(g.colab?.nombre_completo || ''))}
                  </div>
...
function renderResumenBody(g) {
  ...
  document.getElementById('resumenNombre').textContent =
    formatNombreCompleto(g.colab?.nombre_completo || '');
...
async function marcarTodoPagado(colabId) {
  ...
  try {
    ...
    showNotification(
      'success',
      'Pagos Registrados',
      `${ids.length} viaje${ids.length !== 1 ? 's' : ''} de ${formatNombreCompleto(g.colab?.nombre_completo || '')} marcados como pagados.`
    );
