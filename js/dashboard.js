...
async function loadDashboard() {
  // Iniciar reloj Lima en tiempo real
  iniciarRelojLima({
    elFecha: document.getElementById('dashClockFecha'),
    elHora:  document.getElementById('dashClockHora')
  });
...
async function loadKPIs() {
  const today = getLimaISODate(); // fecha actual en Lima YYYY-MM-DD
  ...
  const kpis = [
    ...
    { label: 'Viajes del Mes', value: viajesMes.count || 0, icon: '📊', color: 'teal', footer: new Date().toLocaleString('es-PE', { month: 'long', year: 'numeric', timeZone: 'America/Lima' }) },
    ...
    { label: 'Pagos Realizados', value: pagosRealizados.count + ' viajes', icon: '✅', color: 'green', footer: formatMoney(pagosRealizados.monto) + ' — ' + new Date().toLocaleString('es-PE', { month: 'long', timeZone: 'America/Lima' }) }
  ];
...
async function loadChartViajesDia() {
  // Calcular rango de 30 días en hora Lima
  const endStr   = getLimaISODate(); // YYYY-MM-DD Lima
  const [ey, em, ed] = endStr.split('-').map(Number);
  const startDate = new Date(ey, em - 1, ed - 29, 12, 0, 0);
  const endDate   = new Date(ey, em - 1, ed,      12, 0, 0);
  const pad       = n => String(n).padStart(2, '0');
  const fmtISO    = dt => `${dt.getFullYear()}-${pad(dt.getMonth()+1)}-${pad(dt.getDate())}`;
  ...
  for (let d = new Date(startDate); d <= endDate; d.setDate(d.getDate() + 1)) {
    const key = fmtISO(d);
    counts[key] = 0;
    labels.push(new Date(key + 'T12:00:00').toLocaleDateString('es-PE', {
