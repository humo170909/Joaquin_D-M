...
async function loadKPIs() {
  ...
  const [viajesHoy, viajesSemana, viajesMes, colaboradores, pagoSemanal, pagoMensual] = await Promise.all([
    ...
    calcPagoPeriodo(wStart, wEnd),
    calcPagoPeriodo(mStart, mEnd)
  ]);
  ...
  const kpis = [
    ...
    { label: 'Pago Semanal', value: formatMoney(pagoSemanal), icon: '💵', color: 'amber', footer: `${formatDate(wStart)} - ${formatDate(wEnd)}` },
    { label: 'Pago Mensual', value: formatMoney(pagoMensual), icon: '💰', color: 'red', footer: new Date().toLocaleString('es-PE', { month: 'long', year: 'numeric' }) }
  ];
...
async function calcPagoPeriodo(start, end) {
  const { data } = await db
    .from('viaje_colaboradores')
    .select('pago, viajes!inner(fecha, estado)')
    .gte('viajes.fecha', start)
    .lte('viajes.fecha', end)
    .eq('viajes.estado', 'completado');
  return (data || []).reduce((sum, r) => sum + (parseFloat(r.pago) || 0), 0);
}
