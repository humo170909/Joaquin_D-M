      ...
      <tbody>
        ${page.map(v => `
          <tr>
            ...
            <td>
              <div style="font-weight:600">${escapeHtml(formatNombreCompleto(v.conductor_resguardo) || '-')}</div>
              ${v.placa_trailer ? `<span class="badge badge-secondary" style="margin-top:3px">${escapeHtml(v.placa_trailer)}</span>` : ''}
            </td>
  ...
  input.addEventListener('input', () => {
    ...
    dropdown.innerHTML = available.slice(0, 8).map(c => `
      <div onclick="addPersonal('${c.id}','${escapeHtml(c.nombre_completo)}',${c.pago_por_viaje || 0})"
        ...
        <strong>${escapeHtml(formatNombreCompleto(c.nombre_completo))}</strong>
...
function renderPersonalTags() {
  ...
  selectedPersonal.forEach(p => {
    ...
    chip.innerHTML = `${escapeHtml(formatNombreCompleto(p.nombre))}
  ...
  document.getElementById('modalDetalleBody').innerHTML = `
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:20px">
      <div style="background:var(--light-bg);border-radius:var(--radius-md);padding:14px">
        <div style="font-size:.75rem;color:var(--text-muted);margin-bottom:4px">RESGUARDO</div>
        <div style="font-weight:700">${escapeHtml(formatNombreCompleto(viaje.conductor_resguardo) || 'No registrado')}</div>
      </div>
      <div style="background:var(--light-bg);border-radius:var(--radius-md);padding:14px">
        ...
        <div class="table-wrapper">
          <table>
            ...
            <tbody>
              ${personal.map(p => `
                <tr>
                  <td><strong>${escapeHtml(formatNombreCompleto(p.colaboradores?.nombre_completo || ''))}</strong></td>
