      ...
      <tbody>
        ${data.map((r, i) => `
          <tr id="rowp-${r.id}">
            ...
            <td>
              <div style="font-weight:700">${escapeHtml(r.colaboradores?.nombre_completo || '-')}</div>
...
function openViewPagado(vcId) {
  ...
  document.getElementById('viewPagadoTitle').textContent =
    `Detalle — ${r.colaboradores?.nombre_completo || 'Pago'}`;
...
async function exportExcel() {
  ...
  const rows = pagadosData.map((r, i) => ({
    ...
    'Colaborador':     r.colaboradores?.nombre_completo || '-',
...
async function exportPDF() {
  ...
  doc.text(`Generado: ${formatDateTime(new Date().toISOString())} | Total: ${formatMoney(totalMonto)} (${pagadosData.length} registros)`, 14, 36);
